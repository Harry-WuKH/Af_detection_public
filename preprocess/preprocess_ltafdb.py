...
import numpy as np
import wfdb
from scipy.signal import resample, kaiserord, firwin, sosfiltfilt, butter
...
dataset_name = "LTAFDB1/physionet.org/files/ltafdb/1.0.0"
dataset_dir = os.environ.get("LTAFDB_RAW_DIR", os.path.join(data_path, dataset_name))
dataset_tag = "LTAFDB"                           # used for output file names
...
def clean_token(raw):
    """
    Normalize aux_note text to a canonical rhythm token:
    ...
    txt = raw.decode() if isinstance(raw, (bytes, bytearray)) else str(raw)
    txt = txt.strip()
    if txt.startswith("("): # remove leading '('
        txt = txt[1:]       #(AFIB -> AFIB
    if txt.endswith(")"):   #remove trailing ')'
        txt = txt[:-1]      #AFIB) -> AFIB
