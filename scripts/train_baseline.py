...
from models.classifier import ClassifierHead_2048_256_2, ClassifierHead_512_256_2
...
def make_src_dataloaders(X, y, val_ratio, bs, seed=1337):
    ...
    tr_dl = DataLoader(tr_ds, batch_size=bs, sampler=sampler, shuffle=False, drop_last=True)
...
def main():
    ...
    args.save_ckpt = os.path.join(output_dir, "best_da_model.pth")
    ...
    # ---------- build model ----------
    '''
    backbone = ResNet1D_50_backbone(in_ch=1).to(device)
    feat_dim = 2048 if not hasattr(backbone, "out_dim") else backbone.out_dim

    classifier = ClassifierHead_2048_256_2(in_dim=feat_dim, num_classes=2).to(device)
    '''
    backbone = ResNet1D_18_backbone(in_ch=1).to(device)
    feat_dim = 512 if not hasattr(backbone, "out_dim") else backbone.out_dim

    classifier = ClassifierHead_512_256_2(in_dim=feat_dim, num_classes=2).to(device)
    # ---------- loss / optimizer ----------
